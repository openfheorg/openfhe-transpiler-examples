...
void AES_CBC_encrypt_buffer(OpenFheArrayRef<uint8_t, AES_keyExpSize> CT_RoundKey,
							OpenFheArrayRef<uint8_t, AES_BLOCKLEN> CT_iv, OpenFheArrayRef<uint8_t, MSG_SIZE> CT_InBuffer, unsigned int length)
{
  ...
  for (i = 0; i < length; i += AES_BLOCKLEN) {
	...
	XLS_CHECK_OK(XorWithIv(CT_state, CT_iv, cc)); //note using state , not flattened state like original 
	CT_iv = CT_new_iv;
...
const unsigned int NBUF = 4;
	...
	AES_CBC_encrypt_buffer(CT_roundkey, CT_iv, CT_inbuffer, MSG_SIZE);
	  ...
	  execTime = TOC_MS(t);
	  cout << "AES_CCB_encrypt buffer "<<NBUF<< " blocks time: " << execTime/1000.0 << " s" << std::endl;
