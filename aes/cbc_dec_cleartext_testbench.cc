...
void AES_CBC_decrypt_buffer(EncodedArrayRef<uint8_t, AES_keyExpSize> CT_RoundKey, 	EncodedArrayRef<uint8_t, AES_BLOCKLEN> CT_iv, EncodedArrayRef<uint8_t, MSG_SIZE> CT_InBuffer, unsigned int length)
{
  ...
  EncodedArray<uint8_t, AES_BLOCKLEN> CT_storeNextIv;
  ...
  for (i = 0; i < length; i += AES_BLOCKLEN)
  {
    ...
    XLS_CHECK_OK(XorWithIv(CT_state, CT_iv));
